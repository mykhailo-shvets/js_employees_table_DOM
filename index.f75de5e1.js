...
function validData(key, value, data) {
    if (key === "name") {
        ...
        if ([
            ...value
        ].find((x)=>x.toUpperCase() === x.toLowerCase() && x !== " ")) return pushNotification(450, "error", 'Invalid data, please use name in "Name" field', "error");
    }
    if (key === "position") {
        if (value.trim().length === 0) return pushNotification(450, "error", 'The "Possition" is empty', "error");
        if ([
            ...value
        ].find((x)=>x.toUpperCase() === x.toLowerCase() && x !== " ")) return pushNotification(450, "error", 'Invalid data, please use letters in "Position" field', "error");
    }
    if (data !== undefined) {
        if (!("office" in data)) {
...
tableBody.addEventListener("dblclick", (e)=>{
    ...
    ].findIndex((x)=>x === e.target.closest("td"));
    if (findIndex === 2) {
